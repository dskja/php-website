...
class Page {
    ...
    public function contactSend(Request $request){
        ...
        if(!$request->subject || strlen($request->subject) < 5){
            return (new Response([
                'error' => true,
                'message' => e('Please enter a subject.'),
                'token' => csrf_token()
            ]))->json();
        }
        ...
        $message = 'Name: '.Helper::RequestClean($request->name).'<br>Subject: '.Helper::RequestClean($request->subject).'<br><br>Email: '.Helper::RequestClean($request->email).'<br><br>'.Helper::RequestClean($request->message);
